...
class LoginSpiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        所以 Request 的参数对它适用。FormRequest 有一类方法 from_response 用于快速构建 FormRequest 对象。from_response 方法会从第一步返回的 response 中获取请求的 url，form 表单信息等等，因此只需要指定必要的表单数据和回调函数即可。
        """
